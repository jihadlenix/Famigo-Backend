...
class Redemption(Base):
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid4()))
    reward_id: Mapped[str] = mapped_column(String(36), ForeignKey("reward.id", ondelete="CASCADE"), index=True)
    requested_by_member_id: Mapped[str] = mapped_column(String(36), ForeignKey("familymember.id", ondelete="CASCADE"), index=True)
    approved_by_member_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("familymember.id", ondelete="SET NULL"))
    status: Mapped[RedemptionStatus] = mapped_column(default=RedemptionStatus.REQUESTED, index=True)
    created_at: Mapped["DateTime"] = mapped_column(DateTime(timezone=True), default=utcnow, nullable=False)
    updated_at: Mapped["DateTime"] = mapped_column(DateTime(timezone=True), default=utcnow, onupdate=utcnow, nullable=False)
    redeemed_at: Mapped["DateTime | None"] = mapped_column(DateTime(timezone=True))
    reward: Mapped["Reward"] = relationship(back_populates="redemptions")
    requested_by: Mapped["FamilyMember"] = relationship(back_populates="redemptions", foreign_keys=[requested_by_member_id])
